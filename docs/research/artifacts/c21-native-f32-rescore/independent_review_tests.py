...
class Scoring(FixtureCase):
    def test_equivalent_metadata_decimals_at_real_stream_seam(self):
        expanded = self.rows[0]["public_metadata"]
        fraction = struct.unpack("<f", struct.pack("<f", 1/7))[0]
        shortened = [0.14285715 if v == fraction else v for v in expanded]
        self.assertNotEqual(shortened, expanded)
        self.assertEqual(struct.pack("<4480f", *shortened), struct.pack("<4480f", *expanded))
        rows = [{**r, "public_metadata": shortened} for r in self.rows]
        _, truths, _ = self.validated(rows)
        for control in ("factual", "uniform_attention"):
            values = runtime(rows, control)
            for row in values:
                row["public_metadata"] = expanded
            with mock.patch.object(I, "jsonl", return_value=iter(values)):
                result = I.stream("synthetic", rows, truths, control)
            self.assertEqual(len(result["metrics"]), 768)

    def test_metadata_f32_identity_rejects_ulp_type_and_nonfinite(self):
        row = self.rows[0]; value = runtime([row])[0]
        fraction = struct.unpack("<f", struct.pack("<f", 1/7))[0]
        index = row["public_metadata"].index(fraction)
        bits = struct.unpack("<I", struct.pack("<f", fraction))[0]
        next_float = struct.unpack("<f", struct.pack("<I", bits+1))[0]
        for invalid in (next_float, True, "0.14285715", float("nan"), float("inf"), -float("inf"), 3.5e38):
            changed = copy.deepcopy(value); changed["public_metadata"][index] = invalid
            with self.subTest(value=repr(invalid)), mock.patch.object(I, "jsonl", return_value=iter([changed])):
                with self.assertRaisesRegex(I.Invalid, "metadata|F32"):
                    I.stream("synthetic", [row], [{}], "factual")
        changed = copy.deepcopy(value); changed["public_metadata"].pop()
        with mock.patch.object(I, "jsonl", return_value=iter([changed])):
            with self.assertRaisesRegex(I.Invalid, "array shape"):
                I.stream("synthetic", [row], [{}], "factual")
        changed = copy.deepcopy(value); changed["public_metadata"][1] = -0.0
        with mock.patch.object(I, "jsonl", return_value=iter([changed])):
            with self.assertRaisesRegex(I.Invalid, "metadata"):
                I.stream("synthetic", [row], [{}], "factual")

    def test_metadata_repair_preserves_other_typed_input_identities(self):
        row = self.rows[0]; value = runtime([row])[0]
        mutations = [lambda r: r.__setitem__("policy_label", float(r["policy_label"])),
                     lambda r: r.__setitem__("input_index", False),
                     lambda r: r["public_cells"][0].__setitem__(0, False),
        ...
        for mutate in mutations:
            ...
            with mock.patch.object(I, "jsonl", return_value=iter([changed])):
                with self.assertRaisesRegex(I.Invalid, "public input|exact keys"):
                    I.stream("synthetic", [row], [{}], "factual")
