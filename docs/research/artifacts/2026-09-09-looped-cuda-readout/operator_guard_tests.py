...
class Admission(TempCase):
    ...
    def run_mock(self,config_change=None,report_change=None,state_change=None):
        config=ss.registered('qual-initial-c10_true',self.binding)
        ...
        def supervise(command,root,env,output,telemetry,record,sample,**kwargs):
            ...
            report=dict(config['expected_report'],elapsed_seconds=.01)
            if report_change:report.update(report_change)
            put(root/'report.json',report)
            put(root/'metadata.json',{'provenance':dict(source_revision=self.binding['source'],
        ...
        fake=types.SimpleNamespace(supervise_child=supervise,terminated=lambda *x:None,
            verify=lambda root:'a'*64,reported_model_elapsed=lambda report,limit:report['elapsed_seconds'])
