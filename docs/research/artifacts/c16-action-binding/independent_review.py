#!/usr/bin/env python3
"""Independent C16 finite-data and numerical review. No model or fit is run.
...
ADDED_FIELDS = {"logits", "stage", "loops", "cleared", "query_cleared", "model_input_sha256"}
...
def schedule():
    ...
    indices = np.concatenate(epochs)[:588800].astype("<u4")
    need(sha(indices.tobytes()) == SCHEDULE_SHA, "registered PCG64 schedule identity")
    return indices.reshape(1150, 512).tolist()
...
def dataset(value, visual=None):
    ...
    expected_fit, expected_held, updates = abstract_rows(FIT), abstract_rows(HELD), schedule()
    need(value["schema"] == "looped-action-binding-data-v1" and exact(value["fit"], expected_fit)
         and exact(value["heldout"], expected_held) and exact(value["updates"], updates), "finite dataset or schedule differs")
    exposure = Counter(itertools.chain.from_iterable(updates))
    ...
    audit = {"abstract": {"fit": abstract_audit(expected_fit, exposure), "heldout": abstract_audit(expected_held)},
             ...
             "schedule": {"updates": 1150, "batch": 512, "presentations": 588800, "completed_epochs": 383, "partial_epoch_rows": 512,
                          ...
                          "rows_with_384_presentations": sum(v == 384 for v in exposure.values()), "indices_sha256": SCHEDULE_SHA}}
...
def authority(config):
    keys(config, {"schema", "dataset", "dataset_sha256", "registration", "frozen_files", "streams", "integrity_receipt", "integrity_receipt_sha256"}, "config")
    ...
    need(receipt.get("schema") == "looped-action-binding-integrity-v1" and receipt.get("accepted") is True, "external integrity rejected")
...
def reconstruction(data, records):
    ...
    decision, components = gates(summaries)
...
def review(config_path, report_path):
    ...
    expected = reconstruction(data, config["streams"])
    header = {"schema": "looped-action-binding-analysis-v1", "accepted": True, "classification": "single_seed_abstract_binding_screen",
    ...
    return {"schema": "looped-action-binding-independent-review-v1", "accepted": True,
            ...
            "canonical_orbits": {"fit": 256, "heldout": 128}, "elapsed_seconds": time.monotonic()-started,
            "scope": "Independent finite rows/schedule/orbit reconstruction, cached attention adapter, scalar CE/margins/counts, postclamp feature hashes, paired differences, analytic/missing/constant controls and exact gates. No fitting or model inference. External source/build/initialization/training/gradient/device/profile/checkpoint/cleanup acceptance shares the pinned lifecycle receipt; selected C15 cached producer provenance is inherited."}
