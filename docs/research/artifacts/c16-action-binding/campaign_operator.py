"""Registered C16 sequencing; generic, hash-pinned C12 process supervision."""
import os
...
REPO = Path('/home/stepan/Projects/code/Tofy-action-binding')
...
def admission(report):
    ...
    values = [report['updates_elapsed_seconds'], report['checkpoint_seconds']]
    require(all(type(x) in (int, float) and math.isfinite(x) and x >= 0 for x in values), 'invalid duration')
    predicted = 1.15 * (1150 / 5) * values[0] + values[1]
    return dict(accepted=predicted <= 600, reserved_training_seconds=predicted, measured_confirmation_seconds=values[0], checkpoint_seconds=values[1], model_limit_seconds=600)
...
def prepare(campaign, binary, dataset):
    ...
    for name in ARTIFACTS:
        path = R / name
        require(digest(path) == digest(REPO / 'docs/research/artifacts/c16-action-binding' / name), 'operator snapshot differs')
...
def main():
    ...
    if args.stage == 'prepare':
        ...
        print(f'C16 registered {args.stage} stage complete.')
