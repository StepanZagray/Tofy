...
class EvaluatorTests(unittest.TestCase):
    ...
    def test_common_preserves_root_manifest_with_nonempty_frozen_inputs(self):
        root = self.root / 'qual-initial-c10_true'; root.mkdir()
        binary = self.root / 'learned_readout_probe'; binary.write_bytes(b'fixture binary')
        binary_sha = a.sha_bytes(binary.read_bytes())
        frozen = self.root / 'frozen-input'; frozen.write_bytes(b'parent input, not the root manifest')
        frozen_sha = a.sha_bytes(frozen.read_bytes())
        invocation = self.root / 'invocation.json'
        arguments = ['--mode', 'evaluate-imported', '--max-seconds', '60']
        argv = [str(binary), *arguments, '--output-dir', str(root), '--device', 'cuda:0']
        config = dict(campaign=str(self.root), source='1' * 40, binaries={'learned_readout_probe': binary_sha})
        write_json(invocation, dict(name=root.name, binary=binary.name, arguments=arguments,
                                   frozen_files={str(frozen): frozen_sha}))
        write_json(root / 'metadata.json', dict(exact_args=argv, provenance=dict(source_revision=config['source'],
            candle_graph_revision=a.DEPENDENCY, binary_sha256=binary_sha, features='cudnn,profiling',
            build_command='cargo build --release --locked --features cudnn,profiling,serde_json/float_roundtrip')))
        write_json(root / 'report.json', dict(status='complete_pending_analysis', optimizer_updates=0, elapsed_seconds=.1))
        write_json(root / 'launch.json', dict(exact_args=argv))
        root_sha = seal(root)
        root.with_suffix('.manifest.sha256').write_text(root_sha + '\n')
        moment = '2026-09-09T15:00:00+01:00'
        write_json(root.with_suffix('.exit.json'), dict(accepted=True, returncode=0, pid_gone=True,
            model_pid_gone=True, group_gone=True, owned_survivors=[], group_survivors=[], cleanup_error=None,
            failure=None, report_status='complete_pending_analysis', headroom_mib=1024, max_temperature_c=40,
            owned_pids=[1073741824], owned_process_start_ticks={'1073741824': 1}, manifest_sha256=root_sha,
            reported_model_elapsed_seconds=.1, model_phase_seconds=.2, finalization_seconds=.1, finished_local=moment))
        write_json(root.with_suffix('.process.json'), dict(command=argv, binary_sha256=binary_sha,
            invocation=str(invocation), invocation_sha256=a.sha_bytes(invocation.read_bytes()), started_local=moment,
            environment=dict(NVIDIA_TF32_OVERRIDE='0', TOFY_PERF_TRACE=str(root.with_suffix('.host.json')),
                             NSYS_NVTX_PROFILER_REGISTER_ONLY='0')))
        result = a.common(root, config, head=True, expected_name=root.name)
        self.assertNotEqual(root_sha, frozen_sha)
        self.assertEqual(result['manifest_sha256'], root_sha)
        self.assertEqual(result['manifest_sha256'], result['exit']['manifest_sha256'])
