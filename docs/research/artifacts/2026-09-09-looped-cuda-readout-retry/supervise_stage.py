...
def verify_audit(root, config, binding, command):
    """A CPU population audit records no model spans, tensors, checkpoints or CUDA work."""
    subset(config, dict(name=root.name, stage='audit', binary='looped_agent_probe'))
    exact(command, [str(root.parent / 'looped_agent_probe'), *config['arguments'],
                    '--output-dir', str(root), '--device', 'cpu'], 'audit command')
    require(all(type(item) is str for item in command), 'invalid audit argument type')
    def option(name):
        require(command.count(name) == 1 and command.index(name)+1 < len(command), f'ambiguous audit {name}')
        return command[command.index(name)+1]
    require(option('--mode') == 'known-features-audit' and option('--device') == 'cpu'
            and option('--output-dir') == str(root) and command.count('--known-mapping') == 1
            and not any(flag in command for flag in ('--checkpoint','--known-seen','--known-replay',
                '--known-features-heldout','--search')), 'audit is not the CPU-only population mode')
    text = option('--known-features-confirmation-panel')
    require(text in ('0','1','2'), 'invalid audit panel')
    panel = int(text)
    for name, value in (('--seed','0'),('--loops','4'),('--batch','1'),('--effective-batch','1'),
                        ('--eval-episodes','256'),('--data-seed',str(20260917+panel))):
        require(option(name) == value, f'audit population option differs: {name}')
    names = {'known-features-input-audit.jsonl','launch.json','metadata.json','profiles.json','report.json'}
    require(regular_files(root) == names | {'manifest.json'}, 'unexpected CPU audit artifact population')
    pin = root.with_suffix('.manifest.sha256')
    manifest_hash = pin.read_text().strip()
    verify_files({str(pin):digest(pin), str(root/'manifest.json'):manifest_hash})
    manifest = read(root/'manifest.json')
    require(set(manifest) == {'schema','files'} and manifest['schema'] == 'looped-agent-artifacts-v1'
            and type(manifest['files']) is dict and set(manifest['files']) == names,
            'CPU audit manifest schema/files differ')
    verify_files({str(root/name):sha for name,sha in manifest['files'].items()})
    profiles = read(root/'profiles.json')
    profile_root, host = root.with_suffix('.profiles'), root.with_suffix('.host.json')
    exact(profiles, dict(root=str(profile_root), files={},
        host_trace=dict(path=str(host),sha256=digest(host)),
        nsight='external capture and export; bind in a separate bundle after profiler exit'), 'CPU audit profiles')
    verify_files({str(host):profiles['host_trace']['sha256']})
    exact(read(host), [], 'CPU audit host trace')
    require(not profile_root.is_symlink() and (not profile_root.exists()
            or (profile_root.is_dir() and not list(profile_root.iterdir()))), 'CPU audit contains model profile artifacts')
    for suffix in ('.nsight','.bound'):
        path = root.with_suffix(suffix)
        require(not path.exists() and not path.is_symlink(), 'CPU audit contains unsupported CUDA/bound artifacts')
    report, metadata, launch = (read(root/name) for name in ('report.json','metadata.json','launch.json'))
    subset(report, dict(schema='looped-known-features-v1', status='complete_pending_analysis',
        evidence_class='data_audit', optimizer_updates=0, model_forwards=0, input_rows=256,
        fit_rows=0, eval_rows=256, layouts=256, confirmation_panel=panel,
        partition='confirmation_eval', data_seed=20260917+panel,
        episode_id_base=0x43554441434f4e46+panel*0x10000))
    artifact = root/'known-features-input-audit.jsonl'
    exact(report['artifacts'], [dict(file=artifact.name,bytes=artifact.stat().st_size,sha256=digest(artifact))],
          'CPU audit reported artifacts')
    subset(metadata['provenance'], dict(source_revision=binding['source'],
        binary_sha256=binding['binaries']['looped_agent_probe'], candle_graph_revision=DEPENDENCY, checkpoint=None))
    subset(metadata, dict(exact_args=command, seed=0, data_seed=20260917+panel,
                          physical_batch=1, effective_batch=1, accumulation=1))
    subset(metadata['known_features'], dict(schema='looped-known-features-v1', optimizer_updates=0,
        layouts=256, fit_layouts=0, eval_layouts=256, loops=4, condition='factual',
        implementation_smoke=False, confirmation_panel=panel, partition='confirmation_eval',
        cuda_gemm_reduced_precision_f32=None))
    subset(launch, dict(exact_args=command, source_revision=binding['source'],
...
def main():
    ...
    try:
        state['manifest_sha256'] = verify_audit(root, config, binding, command) if audit else lifecycle.verify(root)
