"""C19 frozen native campaign; hash-pinned generic process/profile lifecycle."""
import os
...
REPO=Path('/home/stepan/Projects/code/Tofy-native-binding')
...
def prepare(campaign,binary):
    ...
    for name in ARTIFACTS:
        p=R/name;require(digest(p)==digest(REPO/'docs/research/artifacts/c19-native-binding'/name),'source archive differs');files[str(p)]=digest(p)
...
def main():
    ...
    if a.stage=='prepare':
        ...
        require(digest(a.campaign/'launch-spec.json')==a.spec_sha256,'launch spec changed');spec=read(a.campaign/'launch-spec.json');(execute if a.stage=='execute' else analyze)(spec);print(f'C19 {a.stage} complete.')
