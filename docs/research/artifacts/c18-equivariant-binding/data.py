#!/usr/bin/env python3
"""C18 unchanged finite inputs, with the frozen C17 presentations regrouped.
...
SCHEMA = "looped-action-binding-data-v2"
...
SEED, PRESENTATIONS = 20260922, 588800
...
C17_FLAT_SHA = "a46eac4347bfe16d55d220240452cb67643ccd007a97d2e56da16be45cb6f51e"
...
def batch_size(value):
    require(type(value) is int and 512 <= value <= 32768 and value & (value - 1) == 0,
            "batch must be a power of two in512..32768")
    return value


def flat_schedule():
    rng = np.random.Generator(np.random.PCG64(SEED))
    flat = np.concatenate([rng.permutation(1536) for _ in range(384)])[:PRESENTATIONS]
    require(sha(flat.astype("<u4").tobytes()) == C17_FLAT_SHA, "C17 presentation sequence drift")
    return flat


def schedule(effective_batch, kind="training", smoke_updates=None):
    batch_size(effective_batch)
    require(kind in ("training", "smoke"), "schedule kind")
    require((kind == "training" and smoke_updates is None) or
            (kind == "smoke" and type(smoke_updates) is int and smoke_updates in (2, 5)),
            "training has no update override; smoke requires2 or5 updates")
    count = PRESENTATIONS if kind == "training" else effective_batch * smoke_updates
    flat = flat_schedule()[:count]
    return [flat[start:start + effective_batch].tolist() for start in range(0, count, effective_batch)]
...
def dataset(effective_batch, kind="training", smoke_updates=None, visual=None, visual_audit=None):
    fit, held = abstract_rows(FIT), abstract_rows(HELD)
    updates = schedule(effective_batch, kind, smoke_updates)
    flat = np.asarray([i for batch in updates for i in batch], dtype="<u4")
    count, indices_sha = len(flat), sha(flat.tobytes())
    result = dict(schema=SCHEMA, fit=fit, heldout=held, updates=updates, cached_visual=visual or [],
                  schedule={"kind": kind, "effective_batch": effective_batch,
                            "presentations": count, "indices_sha256": indices_sha},
    ...
    for name, rows in (("fit", fit), ("heldout", held)):
        ...
        orbits = []
        exposure = np.bincount(flat, minlength=1536) if name == "fit" else None
    ...
    counts = np.bincount(flat, minlength=1536)
    result["audit"]["schedule"] = {"updates": len(updates), "batch": effective_batch, "presentations": count,
        "full_updates": sum(len(batch) == effective_batch for batch in updates),
        "tail_rows": count % effective_batch,
        "completed_epochs": count // 1536, "partial_epoch_rows": count % 1536, "minimum_presentations_per_row": int(counts.min()),
...
def validate(value, visual=None, visual_audit=None):
    require(type(value) is dict and set(value) == {"schema", "fit", "heldout", "updates", "cached_visual", "provenance", "audit", "schedule"}, "dataset schema")
    spec = value["schedule"]
    require(type(spec) is dict and set(spec) == {"kind", "effective_batch", "presentations", "indices_sha256"}, "schedule schema")
    expected = dataset(spec["effective_batch"], spec["kind"], len(value["updates"]) if spec["kind"] == "smoke" else None,
                       visual, visual_audit)
...
def main():
    ...
    parser.add_argument("--kind", required=True, choices=("training", "smoke"))
    parser.add_argument("--updates", type=int, choices=(2, 5))
    ...
    schedule(args.batch, args.kind, args.updates)  # Reject ambiguous CLI before cache access.
    require(file_sha(C17_DATA) == C17_DATA_SHA, "frozen C17 dataset binding")
    visual, visual_audit = load_visual()
    result = dataset(args.batch, args.kind, args.updates, visual, visual_audit)
