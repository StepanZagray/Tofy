...
//! `text_reply`, `code`, `done`, and legacy `fetch_docs`.
...
pub enum Action {
    ...
    Done,
    /// Legacy docs action. Go-focused training maps this back to code.
    FetchDocs,
}
...
#[cfg(test)]
mod tests {
    use super::code_request_score;

    #[test]
    fn go_function_prompts_score_as_code_requests() {
        let prompt =
            "Return only Go code. Implement exactly this function:\nfunc Add(a int, b int) int";
        assert!(code_request_score(prompt) >= 2.0);
    }
}
