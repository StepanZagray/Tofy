...
/// Format full conversation so the encoder sees prior assistant (text/code) and user turns.
/// Enables cross-action awareness: text decoder can describe what code did, code decoder sees what was said.
...
pub async fn run(
    ...
    tracing::info!("Tofy OpenAI-compatible server listening on http://{}", bind);
    tracing::info!("  POST /v1/chat/completions  — chat completions");
    tracing::info!("  GET  /v1/models            — list models (tofy)");
