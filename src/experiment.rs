...
//!
//! The P1 command-line harness (`p1a` / `p1b` / `p1c` / `p1c-hard`) lives only on
//! the `p1` git branch. The P1 exact simulator itself remains on `main` in
//! `domain.rs` / `generator.rs` / `search.rs`, because the P2 curriculum
//! generates its factual transitions from it.

use crate::p2::cli::{
    run_p2_arc3_bridge, run_p2_arc3_eval, run_p2_bf16_bench, run_p2_bf16_drift, run_p2_eval,
    run_p2_train, P2Arc3BridgeArgs, P2Arc3EvalArgs, P2Bf16BenchArgs, P2Bf16DriftArgs, P2EvalArgs,
    P2TrainArgs,
};
