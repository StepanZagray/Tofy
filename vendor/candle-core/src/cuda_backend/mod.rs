...
impl CudaStorage {
    ...
    pub(crate) fn conv2d_bwd_filter(
        ...
        let dst_el = params.c_out * params.c_in * params.k_h * params.k_w;
        let slice = match (&self.slice, &grad.slice) {
            (S::U8(inp), S::U8(dy)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let dy = &dy.slice(grad_l.start_offset()..);
                let mut dw = device.alloc_zeros::<u8>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_filter::<u8, u8>(
                    inp, inp_l, dy, grad_l, &mut dw, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::U8(dw)
            }
            (S::BF16(inp), S::BF16(dy)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let dy = &dy.slice(grad_l.start_offset()..);
                let mut dw = device.alloc_zeros::<bf16>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_filter::<bf16, f32>(
                    inp, inp_l, dy, grad_l, &mut dw, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::BF16(dw)
            ...
            (S::F16(inp), S::F16(dy)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let dy = &dy.slice(grad_l.start_offset()..);
                let mut dw = device.alloc_zeros::<f16>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_filter::<f16, f16>(
                    inp, inp_l, dy, grad_l, &mut dw, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F16(dw)
            ...
            (S::F32(inp), S::F32(dy)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let dy = &dy.slice(grad_l.start_offset()..);
                let mut dw = device.alloc_zeros::<f32>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_filter::<f32, f32>(
                    inp, inp_l, dy, grad_l, &mut dw, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F32(dw)
            ...
            (S::F64(inp), S::F64(dy)) => {
                let inp = &inp.slice(inp_l.start_offset()..);
                let dy = &dy.slice(grad_l.start_offset()..);
                let mut dw = device.alloc_zeros::<f64>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_filter::<f64, f64>(
                    inp, inp_l, dy, grad_l, &mut dw, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F64(dw)
            }
            _ => Err(CudaError::InternalError("dtype mismatch in conv2d_bwd_filter"))?,
        };
    ...
    pub(crate) fn conv2d_bwd_data(
        ...
        if !kernel_l.is_contiguous() {
            Err(CudaError::InternalError(
                "conv2d_bwd_data requires a contiguous kernel",
            ))?
        }
        let dst_el = params.b_size * params.c_in * params.i_h * params.i_w;
        let slice = match (&self.slice, &kernel.slice) {
            (S::U8(dy), S::U8(k)) => {
                let dy = &dy.slice(grad_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
                let mut dx = device.alloc_zeros::<u8>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_data::<u8, u8>(
                    dy, grad_l, k, &mut dx, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::U8(dx)
            }
            (S::BF16(dy), S::BF16(k)) => {
                let dy = &dy.slice(grad_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
                let mut dx = device.alloc_zeros::<bf16>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_data::<bf16, f32>(
                    dy, grad_l, k, &mut dx, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::BF16(dx)
            ...
            (S::F16(dy), S::F16(k)) => {
                let dy = &dy.slice(grad_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
                let mut dx = device.alloc_zeros::<f16>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_data::<f16, f16>(
                    dy, grad_l, k, &mut dx, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F16(dx)
            ...
            (S::F32(dy), S::F32(k)) => {
                let dy = &dy.slice(grad_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
                let mut dx = device.alloc_zeros::<f32>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_data::<f32, f32>(
                    dy, grad_l, k, &mut dx, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F32(dx)
            ...
            (S::F64(dy), S::F64(k)) => {
                let dy = &dy.slice(grad_l.start_offset()..);
                let k = &k.slice(kernel_l.start_offset()..);
                let mut dx = device.alloc_zeros::<f64>(dst_el)?;
                crate::cudnn::launch_conv2d_bwd_data::<f64, f64>(
                    dy, grad_l, k, &mut dx, params, &device,
                )
                .map_err(crate::Error::wrap)?;
                S::F64(dx)
            }
            _ => Err(CudaError::InternalError("dtype mismatch in conv2d_bwd_data"))?,
        };
        Ok(Self { slice, device })
    }
}
