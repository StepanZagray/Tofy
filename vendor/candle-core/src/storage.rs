...
impl Storage {
    ...
    pub(crate) fn conv2d_bwd_filter(
        ...
        self.same_device(grad, "conv2d_bwd_filter")?;
        self.same_dtype(grad, "conv2d_bwd_filter")?;
        match (self, grad) {
            (Storage::Cuda(inp), Storage::Cuda(grad)) => {
                let s = inp.conv2d_bwd_filter(l, grad, grad_l, params)?;
                Ok(Self::Cuda(s))
            }
            (lhs, rhs) => Err(Error::DeviceMismatchBinaryOp {
                lhs: lhs.device().location(),
                rhs: rhs.device().location(),
                op: "conv2d_bwd_filter",
            }
            .bt()),
        }
    }
    ...
    pub(crate) fn conv2d_bwd_data(
        ...
        self.same_device(grad, "conv2d_bwd_data")?;
        self.same_device(kernel, "conv2d_bwd_data")?;
        self.same_dtype(grad, "conv2d_bwd_data")?;
        self.same_dtype(kernel, "conv2d_bwd_data")?;
        match (grad, kernel) {
            (Storage::Cuda(grad), Storage::Cuda(kernel)) => {
                let s = grad.conv2d_bwd_data(grad_l, kernel, kernel_l, params)?;
                Ok(Self::Cuda(s))
            }
            (lhs, rhs) => Err(Error::DeviceMismatchBinaryOp {
                lhs: lhs.device().location(),
