#!/usr/bin/env bash
# Worst-case A40 population/VRAM probe for world-core-v2.
set -euo pipefail
...
probe_root="${P2_V2_PROBE_ROOT:-$repo_root/runs/p2/world-core-v2-probe}"
...
"$tofy_bin" p2-train --device cuda:0 --seed 1 --world-core-v2 --spatial-action-field \
  ...
  --spatial-variance-weight 0.05 --spatial-covariance-weight 0.005 \
  --pooled-variance-weight 0.05 --pooled-covariance-weight 0.005 \
  ...
  > >(tee "$probe_root/train.log") 2>&1
finished="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
nvidia-smi --query-gpu=timestamp,index,name,memory.used,memory.total,utilization.gpu,power.draw \
  --format=csv,noheader,nounits >"$probe_root/gpu-after.csv"

jq -e --argjson physical "$physical_batch" \
  '.world_core_schema == "world_core_v2" and .physical_batch == $physical
   and .grad_accum == 1 and .global_step == 2' "$probe_root/train_report.json" >/dev/null
jq -nc --arg schema p2.world_core_v2_batch_probe.v1 --arg status passed \
  ...
  '{schema:$schema,status:$status,started_utc:$started_utc,finished_utc:$finished_utc,
    world_core_schema:"world_core_v2",physical_batch:$physical_batch,grad_accum:1,global_step:2,
...
printf 'passed world-core-v2 batch probe: %s\n' "$probe_root/probe.json"
