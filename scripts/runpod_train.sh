...
if [[ "${TOFY_RUNPOD_TMUX_CHILD:-0}" != "1" && -z "${TMUX:-}" ]]; then
  exec tmux new -s "$SESSION" \
    "TOFY_RUNPOD_TMUX_CHILD=1 PROFILE='$PROFILE' RESUME_TARGET='$RESUME_TARGET' LOG_PATH='$LOG_PATH' TOFY_REPO_DIR='$REPO_DIR' SKIP_GIT_PULL='${SKIP_GIT_PULL:-0}' TOFY_AUTO_STOP='${TOFY_AUTO_STOP:-1}' TOFY_RUNPOD_STOP_WRAPPER='$STOP_WRAPPER' bash '$0' '$MODE'"
fi
