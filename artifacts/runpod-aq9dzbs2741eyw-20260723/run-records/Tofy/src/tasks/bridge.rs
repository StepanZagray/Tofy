...
fn qwen_batch(
    ...
    let encoded = rows
        ...
        .map(|row| {
            ...
            // Always reserve one completion token and EOS. The previous
            // head-only truncation could retain a long prompt while silently
            // dropping the complete supervision target, then mark a prompt
            // token as a completion label.
            let max_prompt = max_seq.saturating_sub(2);
            ...
            let completion_budget = max_seq.saturating_sub(prompt_len + 1).max(1);
            ids.extend(completion.get_ids().iter().take(completion_budget));
            ids.push(qwen_eos_id(tokenizer)?);
            let prompt_len = prompt_len.min(ids.len().saturating_sub(2));
            let semantic_spans = semantic_completion_spans(&row.completion);
...
fn train(args: BridgeArgs) -> Result<()> {
    ...
    let validation_function_max =
        env_usize("TOFY_BRIDGE_VALIDATION_FUNCTION_MAX", SEEN_FUNCTION_MAX);
    println!(
        "Bridge regime={} train_rows={} val_rows={} function_split=train:1-{train_function_max} val:{}-{validation_function_max} heldout_task_rows=0 full_world_grounding=true",
        regime.as_str(),
        train_rows.len(),
        val_rows.len(),
    ...
    for step in (start_step + 1)..=args.steps {
        ...
        for micro_step in 0..grad_accum {
            ...
            let mut cond = if lora_mode {
                ...
                let state_slots = if unfreeze_world {
                    state_slots
                } else {
                    state_slots.detach()
                };
        ...
        if step % val_every == 0 || step == args.steps {
            ...
            tb.flush();
            println!(
                "bridge val step={step} val_ce_matched={:.4} val_ce_zeroed={:.4} val_ce_wrong={:.4} val_semantic_ce_matched={:.4} val_semantic_ce_wrong={:.4} zero_gap={zero_gap:.4} semantic_gap={semantic_gap:.4}",
                losses.matched,
                losses.zeroed,
                losses.wrong,
    ...
    if requires_semantic_gap && best_semantic_gap < min_semantic_gap {
        tb.finish()?;
        bail!(
            "no bridge checkpoint reached the required semantic conditioning gap: selected={best_semantic_gap:.4}, required={min_semantic_gap:.4}; latest={}",
            latest_path.display()
        );
    }
    println!(
